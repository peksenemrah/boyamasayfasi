...
export default function ColoringEngine({ svgDosya, baslik }: Props) {
  ...
  // containerRef: React bu div'in içini ASLA yönetmez — tamamen imperatif
  ...
  // SVG yükle → container'a doğrudan inject et
  useEffect(() => {
  ...
  // Click handler — container'a addEventListener, React event sistemini atla
  useEffect(() => {
    const container = containerRef.current
    if (!container || !yuklendi) return

    const handler = (e: MouseEvent) => {
      const el = (e.target as Element).closest('[data-region]') as SVGElement | null
      if (!el) return
  ...
  const geriAl = useCallback(() => {
    ...
    containerRef.current?.querySelectorAll('[data-region]').forEach(el => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4">
      ...
      <div className="flex gap-4 flex-col md:flex-row">
        ...
        {/* SVG Alanı — React bu div'in içini yönetmez */}
        <div
          ref={containerRef}
          className="flex-1 bg-white border-2 border-dashed border-gray-200 rounded-xl overflow-hidden cursor-crosshair min-h-80"
